...
#initiating Matrixes
m = Array.new(input.lines.map { |l| l.split.map { |n| n.to_i } })
largest =1788696
product=1
for i in 0..19
  for j in 0..19
    if(check_validity(i+3,j))
      product=m[i][j]*m[i+1][j]*m[i+2][j]*m[i+3][j]
      largest=product if product>largest
    end
    if(check_validity(i,j+3))
      product=m[i][j]*m[i][j+1]*m[i][j+2]*m[i][j+3]
      largest=product if product>largest
    end
    if(check_validity(i+3,j+3))
